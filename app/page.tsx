...
export default async function Home() {
  ...
  return <SiteShell><main>
    ...
    <section className="section"><div className="wrap">
      <div className="section-head"><div><div className="eyebrow">Estoque selecionado</div><h2 className="display">Veículos em destaque</h2></div><Link className="thin-link" href="/veiculos">VER ESTOQUE COMPLETO <ArrowRight size={15} /></Link></div>
      {vehicles.length ? <div className="vehicle-grid">{vehicles.slice(0, 3).map(vehicle => <VehicleCard key={vehicle.id} vehicle={vehicle} />)}</div> : <div className="empty"><div className="eyebrow">Estoque</div><h3 className="display">Novas oportunidades chegando</h3><p className="text-muted">Fale com nossa equipe para conhecer os veículos disponíveis.</p><a className="btn" href={site.whatsapp}>Consultar estoque</a></div>}
    </div></section>
