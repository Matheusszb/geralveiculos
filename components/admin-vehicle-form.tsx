...
export function AdminVehicleForm({ vehicle }: { vehicle?: Vehicle }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget); const db = createClient(); const brand = String(form.get('brand')); const model = String(form.get('model'));
    const data = { slug: `${brand}-${model}-${form.get('year')}`.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, ''), brand, model, version: String(form.get('version') || '') || null, year: Number(form.get('year')), model_year: Number(form.get('model_year')), price: Number(form.get('price')), mileage: Number(form.get('mileage')) || null, transmission: String(form.get('transmission') || '') || null, fuel: String(form.get('fuel') || '') || null, color: String(form.get('color') || '') || null, doors: Number(form.get('doors')) || null, plate_end: String(form.get('plate_end') || '') || null, description: String(form.get('description') || '') || null, features: options.filter(option => form.get(`feature-${option}`) === 'on'), status: String(form.get('status')) as Vehicle['status'], featured: form.get('featured') === 'on', show_when_sold: form.get('show_when_sold') === 'on' };
    let id = vehicle?.id;
    if (id) { const { error } = await db.from('vehicles').update(data).eq('id', id); if (error) { alert(error.message); setBusy(false); return; } }
    else { const { data: created, error } = await db.from('vehicles').insert(data).select('id').single(); if (error || !created) { alert(error?.message || 'Não foi possível salvar'); setBusy(false); return; } id = created.id; }
    ...
    for (let index = 0; index < gallery.length; index += 1) {
      ...
      if (item.file) { const path = `${id}/${crypto.randomUUID()}-${item.file.name}`; const { error } = await db.storage.from('vehicle-images').upload(path, item.file); if (error) { alert(`Não foi possível enviar ${item.file.name}`); continue; } const { data: publicUrl } = db.storage.from('vehicle-images').getPublicUrl(path); const uploaded = { ...item, url: publicUrl.publicUrl }; await db.from('vehicle_images').insert({ vehicle_id: id, url: uploaded.url, position: index, object_position }); finalGallery.push(uploaded); }
      else { await db.from('vehicle_images').update({ position: index, object_position }).eq('id', item.id!); finalGallery.push(item); }
    }
  ...
  return <form className="form-card" onSubmit={submit}><div className="form-fields">
    ...
    <label className="full">Descrição<textarea name="description" defaultValue={vehicle?.description || ''} /></label>
    <div className="full image-upload"><label>Fotos do veículo<input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={event => addFiles(event.target.files)} /></label><small>Escolha as fotos e organize a sequência. A primeira sempre será a capa do veículo.</small></div>
    ...
    <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}><input type="checkbox" name="featured" defaultChecked={vehicle?.featured} /> Exibir como destaque na home</label><label style={{ display: 'flex', gap: 8, alignItems: 'center' }}><input type="checkbox" name="show_when_sold" defaultChecked={vehicle?.show_when_sold} /> Exibir se vendido</label>
